...
const Dim = ({ ax,ay,bx,by,val,gap=-24,orient="h" }) => {
  const label = `${Math.round(val)}`;
  if (orient==="h") {
    const ey=ay+gap;
    return <g>
      <line x1={ax} y1={ay+(gap<0?-2:2)} x2={ax} y2={ey+(gap<0?4:-4)} stroke={C.dim} strokeWidth={0.5}/>
      <line x1={bx} y1={by+(gap<0?-2:2)} x2={bx} y2={ey+(gap<0?4:-4)} stroke={C.dim} strokeWidth={0.5}/>
      <JisArrow x1={ax} y1={ey} x2={bx} y2={ey} col={C.dim}/>
      <JisArrow x1={bx} y1={ey} x2={ax} y2={ey} col={C.dim}/>
      <text x={(ax+bx)/2} y={ey+(gap<0?-3:10)} textAnchor="middle" fill={C.dim} fontSize={8} fontFamily={MONO}>{label}</text>
    </g>;
  } else {
    const ex=ax+gap;
    return <g>
      <line x1={ax+(gap<0?-2:2)} y1={ay} x2={ex+(gap<0?4:-4)} y2={ay} stroke={C.dim} strokeWidth={0.5}/>
      <line x1={bx+(gap<0?-2:2)} y1={by} x2={ex+(gap<0?4:-4)} y2={by} stroke={C.dim} strokeWidth={0.5}/>
      <JisArrow x1={ex} y1={ay} x2={ex} y2={by} col={C.dim}/>
      <JisArrow x1={ex} y1={by} x2={ex} y2={ay} col={C.dim}/>
      <text x={ex+(gap<0?-4:10)} y={(ay+by)/2+3} textAnchor="middle" fill={C.dim} fontSize={8} fontFamily={MONO}
        transform={`rotate(-90,${ex+(gap<0?-4:10)},${(ay+by)/2+3})`}>{label}</text>
    </g>;
  }
};
...
function Drawing2D({ data, svgRef }) {
  ...
  const SVG_W=1000, SVG_H=780, TBH=70;
  const dH=SVG_H-TBH, MAR=52;
  ...
  return (
    <svg ref={svgRef} width={SVG_W} height={SVG_H} viewBox={`0 0 ${SVG_W} ${SVG_H}`}
      ...
      {/* 寸法 */}
      <Dim ax={fOX} ay={fOY} bx={fOX+fW} by={fOY} val={OW} gap={-28}/>
      <Dim ax={fOX+fW} ay={fOY} bx={fOX+fW} by={fOY+fH} val={OH} gap={34} orient="v"/>

      {/* ── 平面図 ── */}
      {sortedComps.map((c,i)=><CompTop key={i} comp={c} ox={tOX} oy={tOY} sc={scT} totalD={OD}/>)}
      <OutlineRect x={tOX} y={tOY} w={tW} h={tD}/>
      <Dim ax={tOX} ay={tOY} bx={tOX+tW} by={tOY} val={OW} gap={-22}/>
      <Dim ax={tOX+tW} ay={tOY} bx={tOX+tW} by={tOY+tD} val={OD} gap={26} orient="v"/>

      {/* ── 右側面図 ── */}
      {sortedComps.map((c,i)=><CompSide key={i} comp={c} ox={sOX} oy={sOY} sc={scS} totalH={OH}/>)}
      <OutlineRect x={sOX} y={sOY} w={sW} h={sH}/>
      <Dim ax={sOX} ay={sOY} bx={sOX+sW} by={sOY} val={OD} gap={-22}/>
      <Dim ax={sOX+sW} ay={sOY} bx={sOX+sW} by={sOY+sH} val={OH} gap={26} orient="v"/>
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:SANS,color:C.text}}>
      ...
      <div style={{background:C.panel,borderBottom:`1px solid ${C.border}`,padding:"11px 20px",display:"flex",alignItems:"center",gap:16}}>
        <div>
          ...
          <div style={{fontSize:8,color:C.sub,letterSpacing:2,marginTop:1}}>MOKKOUJYO — PROFESSIONAL DRAWING SYSTEM v9</div>
        </div>
