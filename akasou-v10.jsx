...
// Step1：観察プロンプト（W/D混乱修正版）
const PROMPT_OBSERVE = `このスケッチ画像を観察してください。職人が描いたアイソメ図（立体スケッチ）の場合があります。

【最優先①：タイトル・品名を読む】
画像の上部や余白に書かれた文字（例：「収納BOXの図面」）を全て読み取ってください。

【最優先②：全ての数字を読む（絶対に漏らさない）】
画像に書かれた数字を全て読み取り、リストアップしてください。
★注意：「980」を「80」と誤読しないでください。手書きでも全桁を正確に読んでください。

【アイソメ図の寸法の割り当てルール】
アイソメ図に書かれた複数の数字から W・H・D を判定する方法：

★ステップ1：全ての数字をリストアップする
★ステップ2：垂直方向（縦）に書かれた寸法線の数字 → 高さ（H）
★ステップ3：水平方向に2つある場合 → 大きい方が幅（W）、小さい方が奥行き（D）
★ステップ4：小さい数字（20前後）は板厚

例：数字が「980, 450, 600, 500, 100, 440, 30」の場合
- 600 は右側面の縦寸法 → H=600
- 980 と 450 は水平方向 → 大きい980がW、小さい450がD
- 500, 100, 440 は内部寸法や部分寸法
- 30 は板厚
...
家具の種類：〇〇
数字リスト：（読み取った数字を全て列記）
幅(W)：〇〇mm ← 水平方向の数字のうち大きい方
高さ(H)：〇〇mm ← 縦方向の数字
奥行き(D)：〇〇mm ← 水平方向の数字のうち小さい方
板厚：〇〇mm（不明なら20mm）
扉：あり／なし
棚：あり／なし（あれば枚数）
引き出し：あり／なし
