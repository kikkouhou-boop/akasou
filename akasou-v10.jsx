...
- アイソメ図（立体スケッチ）の場合：
  ・天面（上部）から伸びる2方向の辺の数字のうち、大きい方＝幅(W)、小さい方＝奥行き(D)
  ・縦方向の辺の数字が1つだけならそれが高さ(H)
  ・縦方向の辺の数字が2つある場合（外側と内側で長さの違う寸法線が並んでいる等）は、
    対象物の全体を指す方＝より長い方の寸法線を高さ(H)として採用する。
    もう一方は使わずに「備考」へ「参考寸法：○○mm」として残す
  ・辺の線に沿っていない短い斜め線や小さい目盛り（例：板厚らしき小さい数字）は
    「辺の長さ」ではなく板厚やチリなどの注記である可能性が高い。W/H/Dには使わず「備考」に残す
  ・同じ辺を上端と下端など2箇所で別々に測っている場合は両方をSTEP1に書き出した上で、
    大きい方を主寸法とし、差分を「備考」に「差の可能性：n mm」と記録する
- 不明な項目は「不明」と書く
